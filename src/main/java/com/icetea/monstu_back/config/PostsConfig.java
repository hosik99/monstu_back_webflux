...
public class PostsConfig {
    ...
    public RouterFunction<ServerResponse> postsRouter(PostsHandler handler) {
        return RouterFunctions.nest(path("/posts"),
                        RouterFunctions.route(GET("/").and(accept(MediaType.APPLICATION_JSON)), handler::getAllPosts)
                            .andRoute(GET("/view").and(accept(MediaType.APPLICATION_JSON)), handler::getTopView)
        );
    }
}
